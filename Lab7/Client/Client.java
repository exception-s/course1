...
public class Client {
                    ...
                    try {
                        if (command.isEmpty()) {
                            ...
                            if (!Arrays.asList(commands).contains(command)) {
                                ...
                                Request userRequest = new Request(user, input, org);
                                response = dataProcess(userRequest);
                            } else if (command.equals("execute_script")) {
                                executeScript(input);
                                continue;
                            } else {
                                Request userRequest = new Request(user, input);
                                response = dataProcess(userRequest);
                            }
    ...
    private void userAuthorize() {
        ...
        try {
            while (true) {
                ...
                Request userRequest = new Request(user, new String[]{"check"});
                response = dataProcess(userRequest);
                if (response.getStatus() == Status.USER_IS_REGISTERED) {
                    System.out.println(response.getMessage());
                    if (response.getStatus() != Status.INCORRECT_PASSWORD) {
                        break;
                    }
                } else {
                    System.out.println(response.getStatus());
                    if (response.getStatus() == Status.USER_IS_NOT_REGISTERED) {
                        System.out.println(Status.USER_IS_NOT_REGISTERED);
                        break;
                    } else {
    ...
    private Response dataProcess(Request request) throws IOException, ClassNotFoundException {
    ...
    private void executeScript(String[] input) {
        ...
        final String path = "C:\\Users\\flqme\\IdeaProjects\\course1\\Lab7\\Client\\";
                    ...
                    if (command.isEmpty()) {
                        ...
                        if (!commandList.contains(command)) {
                            ...
                            if (command.equals("add") || command.equals("add_if_min") || command.equals("remove_greater")
                                ...
                                Request request = new Request(user, sb.toString().trim().split("\\s+"));
                                response = dataProcess(request);
                            } else if (command.equals("execute_script")) {
                                executeScript(input);
                                continue;
                            } else {
                                Request request = new Request(user, fileInput);
                                response = dataProcess(request);
                            }
                            if (response.getStatus() == Status.OK) {
                                System.out.println(response.getMessage());
